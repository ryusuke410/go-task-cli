          ...
          for (name2 in options) {
            ...
            if (target !== copy) {
              if (deep && copy && (isPlainObject(copy) || (copyIsArray = isArray(copy)))) {
                ...
                setProperty(target, { name: name2, newValue: extend(deep, clone2, copy) });
...
function fetch(url, opts) {
  ...
  return new fetch.Promise(function(resolve2, reject) {
    ...
    req.on("response", function(res) {
      ...
      const zlibOptions = {
        flush: import_zlib3.default.Z_SYNC_FLUSH,
        finishFlush: import_zlib3.default.Z_SYNC_FLUSH
      };
      if (codings == "gzip" || codings == "x-gzip") {
        body = body.pipe(import_zlib3.default.createGunzip(zlibOptions));
        response = new Response(body, response_options);
        resolve2(response);
        return;
      }
      if (codings == "deflate" || codings == "x-deflate") {
        ...
        raw2.once("data", function(chunk) {
          if ((chunk[0] & 15) === 8) {
            body = body.pipe(import_zlib3.default.createInflate());
          } else {
            body = body.pipe(import_zlib3.default.createInflateRaw());
          }
          response = new Response(body, response_options);
          resolve2(response);
        });
      ...
      if (codings == "br" && typeof import_zlib3.default.createBrotliDecompress === "function") {
        body = body.pipe(import_zlib3.default.createBrotliDecompress());
        response = new Response(body, response_options);
        resolve2(response);
        return;
      }
...
var import_stream, import_http, import_url, import_whatwg_url, import_https, import_zlib3, Readable, BUFFER3, TYPE, Blob, convert, INTERNALS, PassThrough, invalidTokenRegex, invalidHeaderCharRegex, MAP, Headers, INTERNAL, HeadersIteratorPrototype, INTERNALS$1, STATUS_CODES, Response, INTERNALS$2, URL, parse_url, format_url, streamDestructionSupported, Request, URL$1, PassThrough$1, isDomainOrSubdomain, isSameProtocol, lib_default;
var init_lib = __esm({
  "../../node_modules/node-fetch/lib/index.mjs"() {
    "use strict";
...
`;
    ...
    exports2.Gaxios = Gaxios2;
    _a = Gaxios2, _Gaxios_instances = /* @__PURE__ */ new WeakSet(), _Gaxios_urlMayUseProxy = function _Gaxios_urlMayUseProxy2(url, noProxy = []) {
      var _b, _c;
...
var stripAbsolutePath = (path11) => {
  let r = "";
  let parsed = parse3(path11);
...
var prefixPath = (path11, prefix) => {
  if (!prefix) {
    return normalizeWindowsPath(path11);
  }
  path11 = normalizeWindowsPath(path11).replace(/^\.(\/|$)/, "");
  return stripTrailingSlashes(prefix) + "/" + path11;
};
...
var WriteEntry = class extends Minipass {
  ...
  [HEADER]() {
    ...
    (_a = this.onWriteEntry) == null ? void 0 : _a.call(this, this);
...
var lchownSync = (path11, uid, gid) => {
  try {
    return import_node_fs2.default.lchownSync(path11, uid, gid);
  } catch (er) {
    if ((er == null ? void 0 : er.code) !== "ENOENT")
      throw er;
  }
...
var chownrSync = (p, uid, gid) => {
  ...
  try {
    ...
    if ((e == null ? void 0 : e.code) === "ENOENT")
      return;
    else if ((e == null ? void 0 : e.code) === "ENOTDIR" || (e == null ? void 0 : e.code) === "ENOTSUP")
...
var mkdirpManualSync = (path11, options, made) => {
  const parent = (0, import_path4.dirname)(path11);
...
var mkdirpNativeSync = (path11, options) => {
  const opts = optsArg(options);
  opts.recursive = true;
  const parent = (0, import_path6.dirname)(path11);
  if (parent === path11) {
    return opts.mkdirSync(path11, opts);
  }
...
var pathArg = (path11) => {
  ...
  path11 = (0, import_path7.resolve)(path11);
  if (platform3 === "win32") {
    const badWinChars = /[*|"<>?:]/;
    const { root } = (0, import_path7.parse)(path11);
...
var checkCwd = (dir, cb) => {
  import_fs7.default.stat(dir, (er, st) => {
    if (er || !st.isDirectory()) {
      er = new CwdError(dir, (er == null ? void 0 : er.code) || "ENOTDIR");
    }
    cb(er);
  });
};
...
var getDirs = (path11) => {
  const dirs = path11.split("/").slice(0, -1).reduce((set, path12) => {
    ...
    if (s !== void 0) {
      path12 = (0, import_node_path7.join)(s, path12);
    }
    set.push(path12 || "/");
    return set;
  }, []);
...
var PathReservations = class {
  ...
  reserve(paths, fn) {
    ...
    const dirs = new Set(paths.map((path11) => getDirs(path11)).reduce((a, b) => a.concat(b)));
  ...
  #getQueues(fn) {
    ...
    return {
      paths: res.paths.map((path11) => this.#queues.get(path11)),
      dirs: [...res.dirs].map((path11) => this.#queues.get(path11))
    };
  }
  ...
  #clear(fn) {
    ...
    for (const path11 of paths) {
      const q = this.#queues.get(path11);
      if (!q || (q == null ? void 0 : q[0]) !== fn) {
        continue;
      }
      const q0 = q[1];
      if (!q0) {
        this.#queues.delete(path11);
        continue;
      }
      q.shift();
...
var unlinkFile = (path11, cb) => {
  if (!isWindows3) {
    return import_node_fs3.default.unlink(path11, cb);
  }
  const name2 = path11 + ".DELETE." + (0, import_node_crypto.randomBytes)(16).toString("hex");
  import_node_fs3.default.rename(path11, name2, (er) => {
    if (er) {
      return cb(er);
    }
...
var unlinkFileSync = (path11) => {
  if (!isWindows3) {
    return import_node_fs3.default.unlinkSync(path11);
  }
  const name2 = path11 + ".DELETE." + (0, import_node_crypto.randomBytes)(16).toString("hex");
  import_node_fs3.default.renameSync(path11, name2);
  import_node_fs3.default.unlinkSync(name2);
};
var uint32 = (a, b, c) => a !== void 0 && a === a >>> 0 ? a : b !== void 0 && b === b >>> 0 ? b : c;
var cacheKeyNormalize = (path11) => stripTrailingSlashes(normalizeWindowsPath(normalizeUnicode(path11))).toLowerCase();
...
var update = makeCommand(replace.syncFile, replace.asyncFile, replace.syncNoFile, replace.asyncNoFile, (opt, entries = []) => {
  var _a, _b;
  (_b = (_a = replace).validate) == null ? void 0 : _b.call(_a, opt, entries);
  mtimeFilter(opt);
});
var mtimeFilter = (opt) => {
  const filter = opt.filter;
  ...
  opt.filter = filter ? (path11, stat2) => {
    var _a;
    return filter(path11, stat2) && !/* c8 ignore start */
    ((((_a = opt.mtimeCache) == null ? void 0 : _a.get(path11)) ?? stat2.mtime ?? 0) > (stat2.mtime ?? 0));
  } : (path11, stat2) => {
    var _a;
...
var goOsOfNode = (platform6) => {
  if (typeof platform6 !== "string") {
    throw new Error(`Expected string, got ${typeof platform6}`);
  }
  const goOs = platformMapping[platform6];
  if (goOs === void 0) {
    throw new Error(`Unknown platform: ${platform6}`);
  }
  return goOs;
};
...
var TempDir = class _TempDir {
  path;
  constructor(path11) {
    this.path = path11;
  }
  static async create(prefix) {
    const dirPath = await fs11.promises.mkdtemp(path8.join(os.tmpdir(), prefix));
    try {
      const tempDir = new _TempDir(dirPath);
      return tempDir;
    } catch (error) {
      await _TempDir.finalize(dirPath);
      throw error;
    }
  }
  static async finalize(dirPath) {
    if (!fs11.existsSync(dirPath)) {
      return;
    }
    await fs11.promises.rm(dirPath, { recursive: true, force: true });
  }
  async [Symbol.asyncDispose]() {
    await _TempDir.finalize(this.path);
  }
};
...
var hashSha256Verify = (target, checksumsFile) => {
  ...
  const want = checksumEntry.split(/\s+/)[0];
...
var import_gaxios = __toESM(require_src2());
var downloadFile = async (url, destination) => {
  const gaxios = new import_gaxios.Gaxios();
...
var removeGtcFromVersion = (version3) => {
  const parsedVersion = import_semver.default.parse(version3);
  if (!parsedVersion) {
...
var mainBare = async () => {
  ...
  const binDir = path10.join(packageRoot, "task-bin");
  ensureDirSync(binDir);
  const { version: version3 } = readPackageJson(packageJsonPath);
  await installTaskBinary(removeGtcFromVersion(version3), binDir);
};
var installTaskBinary = async (version3, targetDir) => {
  var _stack = [];
  try {
    const owner = "go-task";
    const repo = "task";
    const binary = "task";
    const format = "tar.gz";
    const os2 = goOsOfNode(process.platform);
    const arch = goArchOfNode(process.arch);
    const githubDownload = `https://github.com/${owner}/${repo}/releases/download`;
    const name2 = `${binary}_${os2}_${arch}`;
    const tarball = `${name2}.${format}`;
    const tarballUrl = `${githubDownload}/v${version3}/${tarball}`;
    const checksum = `${binary}_checksums.txt`;
    const checksumUrl = `${githubDownload}/v${version3}/${checksum}`;
    const tempDir = __using(_stack, await TempDir.create("task-"), true);
    const tempTarball = path10.join(tempDir.path, tarball);
    const tempChecksum = path10.join(tempDir.path, checksum);
...
var readPackageJson = (filePath) => {
  const content = fs14.readFileSync(filePath, "utf-8");
  return JSON.parse(content);
};
var ensureDirSync = (path11) => {
  if (fs14.existsSync(path11)) {
    return;
  }
  fs14.mkdirSync(path11, { recursive: true });
};
var main = async () => {
  try {
    await mainBare();
